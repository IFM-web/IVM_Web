...
const PreInviteHistory = async () => {
    ...
    await fetch(localStorage.getItem("Url") + `/AdminArea/GetPreInviteVisitor?Sdate=${fromdate}&Edate=${todate}`).then(response => response.json())
        .then(data => {
            ...
            for (var e of data) {
             ...
             <tr>
                        ...
                        <td>${Masking_Flag == '1' ? masknameany(e.GuestPhone) : e.GuestPhone}</td>
                        <td>${e.GuestEmail}</td>
