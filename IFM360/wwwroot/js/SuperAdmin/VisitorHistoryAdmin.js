...
const VisitorHistoryList = async () => {
    ...
    try {
        ...
        console.log(data);
        const response2 = await fetch(localStorage.getItem("Url") + `/SuperAdmin/GetVisitorHistoryList?Sdate=${startDate}&&Edate=${endDate}`);
...
const checkOut = async (visitorid) => {
    const response = await fetch(localStorage.getItem("Url") + `/SuperAdmin/Checkout?Id=${visitorid}`);
    const data1 = await response.json();
