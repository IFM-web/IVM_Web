...
const DashboardList = async () => {
    ...
    try {
        ...
        const newArr = data.map((e, i) => ({
            ...
            ...(flgs.card_number_flag == 1 && { ["Card No"]: e.CardNo, }),
            ...
            ,
            Action: `<div style="white-space: nowrap; margin:0px; display:flex; text-align:left;" ><button class="btn btn-danger" style="margin-right:10px;" onclick=checkOut(${e.id})>Check Out</button> ${e.Status == 'Accepted' ? `<button style="white-space: nowrap;" class="btn btn-success" onclick="Printpass(${e.id})">Print Pass</button>` : ''}</div>`
        }))
        //console.log(newArr)
...
const checkOut = async (visitorid) => {
    const response = await fetch(`https://ifm360.in/ivmapi/api/VisitorDashboard/VisitorCheckout?emailID=ifmrecp%40gmail.com&password=Admin%40123&Visitorid=${visitorid}`);
    const data = await response.json();
